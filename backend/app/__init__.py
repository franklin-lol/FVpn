...
class User(Base):
    ...
    def traffic_remaining_gb(self) -> Optional[float]:
        """
        Returns None for unlimited (traffic_limit_gb == 0) instead of float('inf').
        Plain JSON (RFC 8259) has no Infinity literal — Starlette's JSONResponse
        uses allow_nan=False, so returning inf here crashes every /api/users and
        /api/nodes response with 'Out of range float values are not JSON compliant'.
        None serializes to JSON `null`, which every client handles cleanly.
        """
